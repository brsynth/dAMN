...
def plot_losses(title, loss_conc, loss_cnst, num_epochs, save=''):
    """
    Generate a high-quality scientific plot comparing Loss_conc and loss_cnst.

    Parameters:
        title (str): Title of the plot.
        loss_conc (array-like): Loss concentration values.
        loss_cnst (array-like): Loss constraints values.
    ...
    # Create secondary Y-axis (right) for loss_cnst
    ax2 = ax1.twinx()
    ax2.set_ylabel('Log(Loss Cosntraints)', color='red')
    ax2.plot(range(1, len(loss_cnst) + 1), loss_cnst, label='Loss Cosntraints', color='red', linestyle='dashed')
    ax2.set_yscale('log')
    ax2.tick_params(axis='y', labelcolor='red')

    # Title with extra padding to move it up
    plt.title(f'Loss OD & Loss Constraints over {num_epochs} Epochs with {title} set', pad=20)
...
class LagNetwork(tf.keras.layers.Layer):
    ...
    """
    def __init__(self, input_dim, hidden_layers=[32], dropout_rate=0.2, name='lag_network', **kwargs):
...
class MetabolicModel(tf.keras.Model):
    ...
    def __init__(self, times, Transport, Stoichiometry, rxn_ids, biomass_rxn_id, 
        ...
        self.weight_stiffness = weight_stiffness # loss weight for constraints
        ...
        self.flux_net = FluxNetwork(self.k, self.n, hidden_layers=hidden_layers_flux, dropout_rate=dropout_rate, name='flux_network')
    ...
    def printout(self):
        ...
        # Print number of trainable parameters for both neural networks
        n_flux = self.flux_net.count_params()
        n_lag = self.lag_net.count_params()
        print(f'Lag network Hidden Layers: {self.hidden_layers_lag} Trainable parameters: {n_lag}')
        print(f'Flux network Hidden Layers: {self.hidden_layers_flux} Trainable parameters: {n_flux}')
        print(f'------------------------------------------------------------------------')
 
    def call(self, C_ref_batch, training=False, verbose=False):
        """
        Unrolls the model over time for a batch of input trajectories.
        Parameters
        ----------
        C_ref_batch : tf.Tensor, shape (batch_size, T+1, k)
            Batch of reference concentration trajectories. The first slice [:, 0, :] is the initial condition.
        training : bool
            Whether the call is in training mode (for dropout etc.)
        verbose : int or bool
            Verbosity for tf.print debugging.
        Returns
        -------
        total_loss_cnst : tf.Tensor (scalar)
            Total loss on stoichiometry and negative fluxes over all time steps.
        total_loss_conc : tf.Tensor (scalar)
            Total loss on concentrations (and biomass drop) over all time steps.
        """
        batch_size = tf.shape(C_ref_batch)[0]
        T = tf.shape(C_ref_batch)[1]
        t_max = tf.cast(T, tf.float32)
        
        # --- Get lag variables for each batch (trajectory) ---
        C0 = C_ref_batch[:, 0, :]   # shape (batch_size, k)
        t_lag, r_lag = self.lag_net(C0, training=training)  # shape (batch_size, 1) each

        # Precompute sigmoid normalization denominator for each batch
        sigmoid = tf.math.sigmoid
        rlag_tlag_2 = (r_lag * t_lag) / 2.0  # shape (batch, 1)
        norm_denom = sigmoid(rlag_tlag_2) - sigmoid(-rlag_tlag_2)  # shape (batch, 1)

        C_pred = C0  # (batch_size, k)
        total_loss_cnst = 0.0
        total_loss_conc = 0.0

        for t in tf.range(T-1):
            # Current absolute time for this step (scalar)
            ttf = tf.cast(t, tf.float32)
            # If self.times is a 1D numpy array, tf.constant is safe

            # Compute fluxes for this step
            v_t = self.flux_net(C_pred, training=training)  # (batch, n)

            # Compute r(t): shape (batch, 1)
            r_arg = r_lag * (ttf - t_lag / 2)
            r_num = sigmoid(r_arg) - sigmoid(-rlag_tlag_2)
            r_t = r_num / norm_denom  # (batch, 1)
            r_t = tf.clip_by_value(r_t, 0.0, 1.0)  # Clamp to [0,1]

            # Broadcast r_t to shape (batch, 1) for update
            r_t_broadcast = tf.reshape(r_t, [-1, 1])  # (batch, 1)

            # Compute change in external concentrations from fluxes; shape: (batch, k)
            delta_C_ext = tf.matmul(v_t, self.Transport, transpose_b=True)
            dt = self.dt_vector[t]
            # Apply reduction for lag phase
            C_next = C_pred + r_t_broadcast * delta_C_ext * dt
            # C_next = C_pred + delta_C_ext * dt # !!! no lag
            ...
            sv_loss = tf.reduce_mean(tf.square(S_v))                    # Stoichiometry should be 0
            neg_flux_penalty = tf.reduce_mean(tf.nn.relu(-v_t))         # No negative fluxes
            neg_conc_penalty = tf.reduce_mean(tf.nn.relu(-C_next))      # No negative concentrations
            constraint_loss = sv_loss + neg_flux_penalty + neg_conc_penalty

            # --- Compute penalty weight ---
            lambda_constraint = 1 / (1 + tf.exp(-self.weight_stiffness * (ttf / t_max - 0.5)))
            ...
            # --- Combine Losses ---
            total_loss_cnst += constraint_loss  # !!! fixed weight
            # total_loss_cnst += constraint_loss * lambda_constraint # !!! decay weight
            total_loss_conc += mse_conc
            ...
            if verbose > 1:
                ...
                tf.print(f'Batch: {i} Time step {t} '
                     ...
                     f'Loss cnst: {total_loss_cnst:.3f} Loss conc: {total_loss_conc:.3f}')

        return total_loss_cnst, total_loss_conc
...
def create_model_train_val(
    ...
    model = MetabolicModel(times, Transport, Stoichiometry, rxn_ids, biomass_rxn_id, 
                           hidden_layers_lag=hidden_layers_lag, hidden_layers_flux=hidden_layers_flux, 
                           dropout_rate=dropout_rate, verbose=verbose)
    ...
    if verbose:
        ...
        dummy = np.zeros((1, model.T+1, model.k), dtype=np.float32)
...
def train_step(model, C_ref_batch, optimizer, verbose=False):
    with tf.GradientTape() as tape:
        loss_cnst, loss_conc = model(C_ref_batch, training=True, verbose=verbose)
        loss = loss_cnst + loss_conc
    gradients = tape.gradient(loss, model.trainable_variables)
    optimizer.apply_gradients(zip(gradients, model.trainable_variables))
    return loss_cnst, loss_conc

def evaluate_model_on_data(model, data_array):
    Z = data_array.shape[0]
    C_all = data_array.reshape((Z, model.T+1, model.k))
    loss_cnst, loss_conc = model(C_all, training=False)
    return loss_cnst.numpy(), loss_conc.numpy()
...
def train_model(
    ...
    losses_cnst_train, losses_conc_train = [], []
    losses_cnst_val, losses_conc_val = [], []
    ...
    for epoch in range(num_epochs):
        ...
        epoch_loss_cnst, epoch_loss_conc = 0.0, 0.0
        for b in range(n_batches):
            start = b * batch_size
            end = min(start + batch_size, Z)
            C_batch = C_train_all[start:end]  # shape: (batch, T+1, k)
            loss_cnst, loss_conc = train_step(model, C_batch, optimizer, verbose=verbose)
            epoch_loss_cnst += loss_cnst.numpy()
            epoch_loss_conc += loss_conc.numpy()

        epoch_loss_cnst /= n_batches
        epoch_loss_conc /= n_batches
        losses_cnst_train.append(epoch_loss_cnst)
        losses_conc_train.append(epoch_loss_conc)

        if C_val_all is not None:
            val_cnst, val_conc = model(C_val_all, training=False)
            val_cnst_np = val_cnst.numpy()
            val_conc_np = val_conc.numpy()
            losses_cnst_val.append(val_cnst_np)
            losses_conc_val.append(val_conc_np)
            val_loss = val_cnst_np + val_conc_np
            print(f'[Epoch {epoch+1}/{num_epochs}] Train cnst: {epoch_loss_cnst:.1e} conc: {epoch_loss_conc:.1e} | '
                  f'Val cnst: {val_cnst_np:.1e} conc: {val_conc_np:.1e}')
            ...
            print(f'[Epoch {epoch+1}/{num_epochs}] Train cnst: {epoch_loss_cnst:.1e} Loss conc: {epoch_loss_conc:.1e}')

    return (losses_cnst_train, losses_conc_train), (losses_cnst_val, losses_conc_val)
